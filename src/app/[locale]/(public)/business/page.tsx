...
export default async function Page({
  ...
  const business = translatedServices.find(
    (service) => service.href === "/business"
  );
  // const businessMenu = navItemsWithHrefs.find(
  //   (menu) => menu.title === "Business"
  // );
  const businessMenu = navItemsWithHrefs[1];
  return (
    <main>
      <PageShell
      ...
      >
        ...
        <div className="grid gap-4 lg:gap-6 sm:grid-cols-2">
          {businessMenu.items?.map(({ items, title }, i) => {
            return (
